...
class RealtimeWebSocketService {
  static final RealtimeWebSocketService _instance = RealtimeWebSocketService._internal();
  ...
  void _setupEventListeners() {
    ...
    _socket?.onConnectError((error) {
      print('Realtime WebSocket connection error: $error');
    ...
    _socket?.on('authenticated', (data) {
      print('Realtime Authentication successful');
    });

    _socket?.on('authentication_failed', (data) {
      print('Realtime Authentication failed: $data');
      onError?.call('Authentication failed');
    });

    _socket?.on('joined_order', (data) {
      print('Realtime Joined order: $data');
    });

    _socket?.on('new_message', (data) {
      print('Realtime New message received: $data');
      onNewMessage?.call(data);
    });

    _socket?.on('location_update', (data) {
      print('Realtime Location update received: $data');
      onLocationUpdate?.call(data);
    });

    _socket?.on('chat_history', (data) {
      print('Realtime Chat history received: ${data['messages']?.length} messages');
    });

    _socket?.on('error', (data) {
      print('Realtime Server error: $data');
